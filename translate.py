...
tree = ET.parse('transResult.data')
root = tree.getroot()
...
if __name__ == '__main__':
    # x = google_trans("这本书很烂",tl="en")
    # print(x)
    for item in root.findall('item'):
        summary = item.find('summary').text
        text = item.find('text').text

        # summary_trans = baidu_translate(summary, fromLang = 'zh', toLang = 'en')
        ...
        tree.write("transResult.data", encoding="utf-8", xml_declaration=True)
